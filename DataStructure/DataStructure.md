...
**무엇인가?** HashTable 또는 HashMap은 **key값을 이용해 데이터를 저장하고 찾는** 자료구조이다. 즉, 키에 대한 해시 값을 사용해 값을 저장하고 조회하며, 키-값 쌍의 개수에 따라 동적으로 크기가 증가하는 Associated Array(연관 배열)를 말한다. 이 Associated Array를 지칭하는 다른 용어로 Map, Dictionary, Symbol Table이 있다. 

**어떻게 동작하는가?** key값을 인덱스로 사용하는 것이 아니라, key값이 hash function을 통해 변환된 **hash값을 인덱스**로 하여 원하는 값을 찾는다. 이유는, 보통 탐색 키는 문자열이거나 매우 큰 숫자인데 반해 테이블 크기는 작으며, 탐색 키 중 일부만 사용되어 전체를 위한 공간을 항상 준비할 필요가 없기 때문. 따라서, 탐색 키를 테이블 크기에 맞는 작은 수로 맵핑시켜야하며, 이 역할은 hash function이 해준다. 

**구현?** 테이블 자체는 배열로 구현한다. 버킷은 충돌을 해결하는 방법에 따라 다르게 구현될 수 있다.

**용도?** 배열의 인덱스를 사용해 접근하기 때문에 **빠른 데이터 검색**에 사용한다.예)디비

*Hashing* 키값을 hash값으로 변환하는것.

**복잡도?** 

### 시간 복잡도와 용도
데이터를 찾을 때, hash function을 한번만 수행하면 array내에 저장된 위치를 바로 접근할 수 있기 때문에, 데이터 저장과 삭제, 검색 속도가 빠르다. 특정 값을 탐색할 때, 인덱스로 접근하므로 평균적으로 시간 복잡도는 O(1). 충돌 때문에 최악의 경우 검색 시간은 O(N).

hash table 검색 성능은 해시 함수의 성능과 테이블 크기에 좌우된다. 충돌이 발생하면 할수록 성능은 점점 O(n)에 가까워지므로 충돌을 최대한 억제시키는 것이 해쉬의 핵심 포인트다.

### Hash Function
좋은 해시 함수란, 데이터를 되도록이면 고르게 분포하여, 충돌을 최소화 할 수 있는 함수이다. 해시테이블의 get(key)와 put(key)에 Cache 로직을 추가하게 되면, 자주 hit하는 데이터에 대해서 바로 데이터를 찾게 함으로써, 성능을 향상 시킬 수 있다. 간단한 방법으로, 탐색키를 테이블의 크기로 나눈 나머지를 해시 주소로 사용하는 방법이 있다. 고르게 분포시키기 위해 테이블 크기는 주로 소수를 선택한다.

**서로 다른 키가 동일한 위치(인덱스)로 해싱(변환)되었을 때 충돌이 발생한다**. 충돌이 자주 발생하면 탐색 시간이 길어져 성능이 저하되므로 해시함수를 수정하거나 충돌 발생시, **해시 테이블의 다른 위치에 저장하거나 테이블 하나의 위치가 여러 개 항목을 저장**할 수 있도록 테이블 구조를 변경한다.
...
#### 2. Open Address 방식
Index에 대한 충돌 처리에 대해서 Linked List와 같은 추가 메모리를 사용하지 않고, hash table의 빈공간을 사용하는 방법으로, Seperate Chaining 방식에 비해서 메모리를 덜 사용한다.

* Linear Probing: 충돌 발생시, 해당 index 다음에 있는버킷들을 순차적으로 탐색한다. 빈 버킷이 나오거나 시작점으로 되돌아 올 때까지.
...
탐사 방식에 따라 해시 성능이 달라지지만, 가장 영향을 미치는 요소는 해시 테이블의 load factor(적재 비율, 전체 버킷 중에서 사용중인 버킷 비율)이다. load factor가 증가할 수록 데이터를 찾거나 삽입하기 위해 필요한 탐사 횟수는 비약적으로(dramatically) 증가한다. 
