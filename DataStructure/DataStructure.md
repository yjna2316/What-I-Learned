...
#### ArrayList
배열로 구현, 인덱스 접근으로 조회가 빠르고 데이터의 추가/삭제는 느리다. 고정된 크기의 배열과 달리, **동적 배열**이라 선언할 때 크기 지정을 안 해줘도 된다.

* Vector와 달리 **동기화**를 보장하지 **않는다.**
* 공간이 부족할 때 추가 공간을 확보한다.(Resizable Array, Dynamic Array)
* 시간 복잡도
  * 읽기: amortized O(1)
  * 쓰기(삽입/삭제): O(N)
...
* 시간 복잡도
  * 읽기: O(N)
  * 쓰기(삽입/삭제): 최선 O(1) 최악 O(N)

**Dynamic Array?** 
새로운 원소를 추가하는데 공간이 없을 떄마다 자동적으로 배열의 크기를 늘려나가는 배열을 말한다. 
* 동작 원리: 공간이 부족하면 보다 기존보다 사이즈가 2배 큰 배열로 원소들을 이동시킴 
* 삽입: amortized O(1), 공간이 부족할때만 O(N)
------------------------------------------------------------
## Stack and Queue
### Stack
...
### 그래프 구현
* 인접행렬: 2차원 배열 사용 
  * vertex간 연결 관계를 상수시간만에 확인 가능
  * 공간 복잡도: O(V*V)
  * 밀집 그래프 표현
* 인접 리스트: 연결리스트 사용
  * vertex의 인접 리스트를 확인해봐야 하므로 vertex간에 연결되어있는지 확인하는데 시간이 걸린다. 
  * 공간 복잡도: O(V + E)
  * 희소 그래프 표현
...
그래프의 모든 정점을 탐색하는 방법
* DFS(깊이 우선 탐색)
  * 한 방향으로만 탐색하다가 더 이상 연결된 정점이 없으면, 바로 이전 단계의 정점으로 돌아가서 다른 방향을 탐색하는 방법
  * 스택 사용
  * 미로 찾기: 갔던 길을 되돌아 오는 상황이 존재
  * 시간 복잡도: O(V + E)

* BFS(너비 우선 탐색)
  * 한 정점과 연결되어 있는 모든 정점을 먼저 방문하는 방법(Tree의 Level Order Traversal 형식)
  * 큐 사용 
  * 원리: 탐색을 시작하는 정점을 큐에 넣는다(enqueue). 그리고 dequeue를 하면서 dequeue한 정점과 간선으로 연결되어 있는 정점들을 enqueue한다.
  * BFS로 구한 경로는 최단 경로
  * 시간 복잡도: O(V + E)
