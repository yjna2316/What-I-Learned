...
### LinkedList
배열의 단점을 보완하기 위한 자료구조로, 각 노드들은 자기 다음에 어떤 노드가 오는지 기억하고 있다. 따라서 이 부분만 다른 값으로 바꿔주면 삭제/삽입을 O(1)만에 할 수 있다. 하지만, 특정 위치에 삭제/삽입을 하고자 한다면 원하는 위치를 탐색하는 과정에서 첫번째 노드부터 순차적으로 접근하기 때문에 O(n) 시간이 추가적으로 발생한다. Array와 달리, 논리적 저장 순서와 물리적 저장 순서가 일치하지 않기 때문이다. 또한 링크 필드에 대한 오버헤드가 생긴다. 결국 연결리스트는 탐색과 삽입, 삭제 모두 O(n)의 시간이 걸리게 된다. Tree 구현에 유용하다.
