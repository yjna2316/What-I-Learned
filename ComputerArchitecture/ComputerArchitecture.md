...
## CPU 구조와 기능
### 명령어 파이프라이닝
CPU가 명령어를 수행할 때 몇가지 단계로 나누어 수행된다. 그리고 각 단계별로 결과값을 저장할 레지스터가 있다. 왜 이렇게 단계별로 나눈걸까? 파이프라이닝 때문이다. 파이프라이닝이란 CPU의 수행을 여러 단계로 나누어, 각 단계를 동시에 수행하는 걸 말한다. 즉, 하나의 명령어 수행이 완전히 끝날 때까지 기다리지 않고 명령어 수행을 단계별로 나누어 각 단계별로 여러 명령어들이 중첩되어 진행되어 여러 개를 동시에 처리해 CPU 성능을 높이는 기술이다. 이론적으로 CPU는 나누어진 단계 수만큼 동시에 명령어를 수행해 CPU 처리 속도를 증가시킬 수 있다.
